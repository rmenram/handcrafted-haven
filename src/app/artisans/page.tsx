    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-12 text-center max-w-3xl mx-auto">
        ...
        <p className="text-lg text-gray-600">
          Discover the talented creators behind our handcrafted treasures. Each artisan brings 
          their unique vision, skill, and passion to every piece they create.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {artisans.map((artisan) => {
          ...
          return (
            <div key={artisan.id} className="bg-card text-card-foreground flex flex-col gap-6 rounded-xl border overflow-hidden hover:shadow-lg transition-shadow">
              <div className="p-6">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {artisan.specialties?.map((specialty) => (
                    <div key={specialty} className="inline-flex items-center justify-center rounded-md border px-2 py-0.5 text-xs font-medium w-fit whitespace-nowrap shrink-0">
                      {specialty}
                    </div>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 mb-4">
                  {artisanProducts.map((product) => (
                    <Link
                      ...
                      className="cursor-default aspect-square rounded-md overflow-hidden bg-gray-100 hover:opacity-75 transition-opacity"
                    >
                      <img
                        src={product.image}
                        alt={product.name}
