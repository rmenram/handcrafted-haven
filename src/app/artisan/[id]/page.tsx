...
export default function ArtisanProfile() {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          // Map _id to id
          const formattedArtisans = artisansData.artisans.map((a: any) => ({
            ...a,
            id: a._id 
          }));
  
          // Map _id to id
          const formattedProducts = productsData.products.map((a: any) => ({
            ...a,
            id: a._id,
  ...
  if (!artisan) {
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        ...
        <button type='button' onClick={() => navigate.push("/artisans")}>Back to Artisans</button>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <button type='button' className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50 px-4 py-3" onClick={() => navigate.back()}>
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex flex-wrap gap-2 justify-center md:justify-start mb-4">
              {artisan.specialties?.map((specialty) => (
                ...
                //   {specialty}
                // </Badge>
                <div key={specialty} className="bg-amber-600 text-white inline-flex items-center justify-center rounded-md border px-2 py-0.5 text-xs font-medium w-fit whitespace-nowrap shrink-0">
                    {specialty}
                </div>
              ))}
            </div>
      ...
      {artisanProducts.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {artisanProducts.map((product) => (
            // Card
            <article key={product.id} className="group overflow-hidden rounded-xl border bg-white shadow-sm hover:shadow-lg transition-shadow duration-300">
              <div className="relative overflow-hidden aspect-square bg-gray-100">
                ...
                {product.featured && (
                  // badge
                  <div className="absolute left-2 top-2 rounded-md bg-amber-500 px-2 py-1 text-xs font-medium text-white shadow-sm">Featured</div>
                )}
              {isCartEnabled && (
                <>
                <button
        ...
        <div className="text-center py-16">
          <p className="text-gray-500 text-lg">This artisan hasn't listed any products yet</p>
        </div>
      )}
    </div>
  );
